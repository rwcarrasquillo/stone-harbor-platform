...
// Load .env.local so the script works locally without manually
// exporting the Supabase vars in the shell.
async function loadEnvLocal() {
  try {
    ...
    // No .env.local — caller must export the vars below.
  }
}
await loadEnvLocal();
...
function parsePlaywrightReport(report) {
  ...
  return {
    ...
    failed,
    skipped,
    // stats.duration is a float in Playwright's JSON reporter
    // (millisecond precision with fractional digits). The
    // test_runs.duration_ms column is integer — round before insert.
    duration_ms:
      typeof report.stats?.duration === "number"
        ? Math.round(report.stats.duration)
        : null,
    details: { tests },
  };
}
